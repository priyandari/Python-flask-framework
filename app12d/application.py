...
def adzanGenerator():
    ...
    while not thread_stop_event.isSet():
        ...
        dictAzan= {'fajr': '','shuruq':'','zuhr':'','asr': '','maghrib':'','isya': '','ddate':'','ttime':''}
        ...
        dictAzan['ddate'] = now.strftime("%d/%m/%Y")
        dictAzan['ttime'] = now.strftime("%H:%M:%S")
        
        socketio.emit('newdata', {'dataAzan': dictAzan, 'fajr': dictAzan['fajr'],'shuruq': dictAzan['shuruq'],'zuhr': dictAzan['zuhr'],'asr': dictAzan['asr'],'maghrib': dictAzan['maghrib'],'isya':  dictAzan['isya'], 'ddate':  dictAzan['ddate'], 'ttime': dictAzan['ttime'] }, namespace='/test')
        socketio.sleep(10)
